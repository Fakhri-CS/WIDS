...
class FeatureSnapshot:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "observed_through", ensure_utc(self.observed_through))
        for name in (
            "subtype_counts",
            "transmitter_counts",
            "receiver_counts",
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "ssid_bssid_counts": [
                ...
                for (ssid_hex, bssid), count in sorted(
                    self.ssid_bssid_counts.items()
                )
            ],
            "channels_by_bssid": {
                key: list(value)
                for key, value in self.channels_by_bssid.items()
            },
            "security_fingerprints_by_bssid": {
                key: list(value)
                for key, value in self.security_fingerprints_by_bssid.items()
            },
            "role_hints_by_transmitter": {
                key: list(value)
                for key, value in self.role_hints_by_transmitter.items()
            },
            "first_seen_by_transmitter": {
...
class _WindowState:
    ...
    def snapshot(self, observed_through: datetime) -> FeatureSnapshot:
        ...
        for group in self.groups.values():
            ...
            roles.setdefault(transmitter, set()).add(role)
            ...
            if bssid is not None and channel is not None:
                channels.setdefault(bssid, set()).add(channel)
            profile = frame.management.security
            if bssid is not None and profile is not None:
                security.setdefault(bssid, set()).add(
                    profile.fingerprint_sha256
                )

        # Retries remain useful for observation timing even though they do not
        # inflate traffic-rate counters.
        for entry in self.entries:
            frame = self.groups[entry.group_key].frame
            transmitter = frame.addresses.transmitter_mac
            timestamp = entry.observed_at
            first_seen[transmitter] = min(
                first_seen.get(transmitter, timestamp),
                timestamp,
            )
            last_seen[transmitter] = max(
                last_seen.get(transmitter, timestamp),
                timestamp,
            )
    ...
    def _increment(self, frame: NormalizedWirelessFrame) -> None:
        ...
        if (
            frame.management.ssid_hex is not None
            and frame.addresses.bssid is not None
        ):
    ...
    def _decrement(self, frame: NormalizedWirelessFrame) -> None:
        self.subtype_counts[frame.frame_subtype.value] -= 1
        self.transmitter_counts[frame.addresses.transmitter_mac] -= 1
        self.receiver_counts[frame.addresses.receiver_mac] -= 1
        if frame.addresses.destination_mac is not None:
...
class FeatureExtractor:
    ...
    def __init__(self, window_seconds: Iterable[float]) -> None:
        ...
        self._states = {
            seconds: _WindowState(seconds=seconds) for seconds in windows
        }
    ...
    def ingest(self, frame: NormalizedWirelessFrame) -> FeatureUpdate:
        """Add a frame and return updated snapshots.

        Sources are required to emit chronological packets. Rejecting an
        out-of-order frame prevents a late packet from corrupting rate windows.
        """

        observed_at = ensure_utc(frame.observed_at)
        with self._lock:
            if (
                self._observed_through is not None
                and observed_at < self._observed_through
            ):
                raise ValueError(
                    "FeatureExtractor requires nondecreasing observed_at values"
                )
            self._observed_through = observed_at
            retry_deduplicated = False
            snapshots: dict[float, FeatureSnapshot] = {}
            for seconds, state in self._states.items():
                state.expire(observed_at)
                retry_deduplicated = (
                    state.add(frame) or retry_deduplicated
                )
    ...
    def snapshots(
        ...
        *,
        ...
        """Read current features, optionally advancing expiry time."""
        ...
        with self._lock:
            ...
            if (
                self._observed_through is not None
                and current < self._observed_through
            ):
    ...
    def reset(self) -> None:
        with self._lock:
            self._states = {
                seconds: _WindowState(seconds=seconds)
                for seconds in self._states
            }
            self._observed_through = None


def _retry_identity(frame: NormalizedWirelessFrame) -> RetryIdentity | None:
...
def _group_key(frame: NormalizedWirelessFrame) -> GroupKey:
